      ...
      child: Obx(() {
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, i) {
            ...
            final summary = it['profileSummary'] as String? ?? (it['skills'] is List ? (it['skills'] as List).join(', ') : 'Service');
            return Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Expanded(
                              child: Text(name, style: const TextStyle(fontWeight: FontWeight.w700, color: MyColors.textPrimary)),
                            ),
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.end,
                              children: [
                                Text('\$${price.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.w700, color: MyColors.textPrimary)),
                                const SizedBox(height: 4),
                                const Text('/hr', style: TextStyle(color: MyColors.textSecondary, fontSize: 12)),
                              ],
                            ),
                          ],
                        ),
                        ...
                        Row(
                          children: [
                            const Icon(Iconsax.star1, color: MyColors.warning, size: 14),
                            const SizedBox(width: 4),
                            Text(rating.toStringAsFixed(1), style: const TextStyle(color: MyColors.textSecondary, fontSize: 12)),
                          ],
                        ),
                      ],
                    ),
...
class _ProviderSkeleton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Container(width: 40, height: 40, decoration: BoxDecoration(color: MyColors.grey10, borderRadius: BorderRadius.circular(20))),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
